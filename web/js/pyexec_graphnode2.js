...
class UnifiedCodeGenerator {
    ...
    generate() {
        ...
        const getUniqueComboValues = () => {
            const comboNameValues = [];
            const uniqueCombos = new Set();

            function getUniqueSeparator(values) {
                const possibleSeparators = [',', ':', ';', '&'];
                for (let separator of possibleSeparators) {
                    if (!values.some(value => value.includes(separator))) {
                        return separator;
                    }
                }
                throw new Error('No unique separator found');
            }
            ...
            processedNodes.forEach(node => {
                node.widgets?.forEach(w => {
                    ...
                    const comboKey = JSON.stringify(values);
                    const separator = getUniqueSeparator(values);
                    if (!uniqueCombos.has(comboKey)) {
                        comboNameValues.push(`# ${name} = ${values.join(separator)}`);
                        uniqueCombos.add(comboKey);
                    }
                });
            });
            return comboNameValues;
        };

        const comboNameValues = getUniqueComboValues();
